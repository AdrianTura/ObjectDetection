...
folder_name = "models/" + net_type + "/" + datetime.now().strftime("%d.%m.%Y-%H:%M:%S")
common_path = folder_name + "/" + net_type 

os.makedirs(folder_name)

model_path = f"{common_path}.onnx"
init_net_path = f"{common_path}_init_net.pb"
init_net_txt_path = f"{common_path}_init_net.pbtxt"
predict_net_path = f"{common_path}_predict_net.pb"
predict_net_txt_path = f"{common_path}_predict_net.pbtxt"

dummy_input = torch.randn(1, 3, 300, 300)
torch.onnx.export(net, dummy_input, model_path, export_params = True, verbose = False, do_constant_folding = True, opset_version=11)
